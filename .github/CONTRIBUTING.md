...
- The solution is `Codebelt.Unitify.slnx`. Central package versions are maintained in `Directory.Packages.props`.
...
- `Codebelt.Unitify.slnx` is the solution used for local development.
...
```powershell
dotnet restore "Codebelt.Unitify.slnx"
dotnet build "Codebelt.Unitify.slnx" --configuration Release --no-restore
```
...
The CI test plan currently runs **1** project(s) and excludes **0** project(s). The workflow also has an optional macOS test job.
...
```powershell
dotnet pack "Codebelt.Unitify.slnx" --configuration Release --no-restore
```
...
By contributing to Unitify API by Codebelt, you agree that your contributions will be licensed under the MIT license.
